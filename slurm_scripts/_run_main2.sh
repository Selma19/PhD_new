...
# This script is useful to move data from the project directory to the ssd local to
# the compute node.
# This transfer should take place after the compute node has been decided (so that
# $LOCAL_TMPDIR is defined) and before the Python program is run.
...
../venv/bin/python ../src/main2.py $1 $2
